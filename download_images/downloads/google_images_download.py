        ...
        while count < limit + 1 and i < len(image_objects):
            ...
            # to be contrary to the implementation details. 
                ...
                if download_status == "success":
                    ...
                    #start: add txt file contains the url for each image
                    abs_path_of_txt_file = '/'.join(absolute_path.split('/')[:-1])
                    if len(abs_path_of_txt_file) > 0:
                        cur_image_name_of_txt_file = '.'.join(return_image_name.split('.')[:-1])
                        if len(cur_image_name_of_txt_file) > 50:
                            cur_image_name_of_txt_file = '.'.join(return_image_name.split('.')[0])
                        try:
                            with open(f'{abs_path_of_txt_file}/{cur_image_name_of_txt_file}.txt', 'w') as f:
                                f.write(f"{object['image_link']}")
