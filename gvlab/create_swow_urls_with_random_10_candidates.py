...
gvlab_random_dataset_path = '/Users/yonatab/PycharmProjects/GVLAB-backend/assets/gvlab_dataset_random_10_candidates.csv'
...
if __name__ == '__main__':
    ...
    df['candidates'] = df['candidates'].apply(lambda x: random.sample(all_images_lst, 10))
    create_urls = df['ID'].apply(lambda x: f"{create_url}/random_10_candidates_{x}")
    ...
    create_urls.to_csv(os.path.join('urls','urls_create_random_10_candidates.csv'),index=False)
    print(f"Wrote URLs CSV: urls_create_random_10_candidates.csv")
    print("Done")